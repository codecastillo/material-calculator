...
const api = {
  ...
  async getMe() {
    const data = await api._fetch('/auth/me');
    currentUser = data.user;
    return data;
  },
  ...
  async getMaterials(supplierId) {
    return api._fetch('/materials/supplier/' + supplierId);
  },
  async createMaterial(supplierId, material) {
    return api._fetch('/materials', {
      method: 'POST',
      body: JSON.stringify({ supplier_id: supplierId, ...material }),
    });
  },
  async updateMaterial(id, data) {
    return api._fetch('/materials/' + id, { method: 'PUT', body: JSON.stringify(data) });
  },
  async deleteMaterial(id) {
    return api._fetch('/materials/' + id, { method: 'DELETE' });
  },

  // Categories
  async getCategories() {
    return api._fetch('/categories');
  },
  async createCategory(name) {
    return api._fetch('/categories', { method: 'POST', body: JSON.stringify({ name }) });
  },
  async deleteCategory(id) {
    return api._fetch('/categories/' + id, { method: 'DELETE' });
  },

  // Jobs
  async getJobs() {
    return api._fetch('/jobs');
  },
  async saveJob(job) {
    return api._fetch('/jobs', { method: 'POST', body: JSON.stringify(job) });
  },
  async deleteJob(id) {
    return api._fetch('/jobs/' + id, { method: 'DELETE' });
  },
  ...
  async getReferralStats() {
    return api._fetch('/auth/referral-stats');
  },

  // Verification
  async verifyEmail(code) {
    return api._fetch('/auth/verify', { method: 'POST', body: JSON.stringify({ code }) });
  },
  async resendCode() {
    return api._fetch('/auth/resend-code', { method: 'POST' });
  },
  ...
  async updateProfile(data) {
    return api._fetch('/auth/profile', { method: 'PUT', body: JSON.stringify(data) });
  },
  async deleteAccount(password, confirm) {
    return api._fetch('/auth/account', {
      method: 'DELETE',
  ...
  async activateLicense(key) {
    return api._fetch('/auth/activate', { method: 'POST', body: JSON.stringify({ key }) });
  },

  // Onboarding
  async getOnboarding() {
    return api._fetch('/onboarding/me');
  },
  async completeOnboarding(data) {
    return api._fetch('/onboarding/complete', { method: 'POST', body: JSON.stringify(data) });
  },
  async getOnboardingToken(token) {
    return api._fetch('/onboarding/token/' + encodeURIComponent(token));
  },
  async redeemOnboarding(body) {
    return api._fetch('/onboarding/redeem', { method: 'POST', body: JSON.stringify(body) });
  },

  // Stripe
  async createCheckout(plan) {
    return api._fetch('/stripe/checkout', { method: 'POST', body: JSON.stringify({ plan }) });
  },
  ...
  async bulkPriceUpdate(supplier_id, percentage, category_id) {
    ...
    return api._fetch('/materials/bulk-price-update', {
      method: 'POST',
  ...
  // Admin
  async getUsers() {
    return api._fetch('/admin/users');
  },
  async updateUser(id, data) {
    return api._fetch('/admin/users/' + id, { method: 'PUT', body: JSON.stringify(data) });
  },
  async deleteUser(id) {
    return api._fetch('/admin/users/' + id, { method: 'DELETE' });
  },
  async getKeys() {
    return api._fetch('/admin/keys');
  },
  async generateKeys(opts) {
    return api._fetch('/admin/keys', { method: 'POST', body: JSON.stringify(opts) });
  },
  async deleteKey(id) {
    return api._fetch('/admin/keys/' + id, { method: 'DELETE' });
  },
  async getAdminStats() {
    return api._fetch('/admin/stats');
  },
};
...
async function doForgotPassword() {
  ...
  if (!email) {
    msgEl.textContent = 'Enter your email';
    msgEl.style.display = '';
...
async function doResetPassword() {
  ...
  if (!code || !password) {
    msgEl.textContent = 'Enter code and new password';
    msgEl.style.display = '';
    msgEl.style.background = '';
    msgEl.style.color = '';
    return;

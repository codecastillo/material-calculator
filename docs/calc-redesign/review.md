...
- **Spec says:** §9 mobile rules trigger at `< 768px` (explicitly stated: "At narrow widths (`< 768px`)").
- **Code has:** `styles.css:977` and `styles.css:982` use `@media(max-width:1100px)` and `@media(max-width:900px)`. No 768px breakpoint exists.
...
- **Code has:** `styles.css:962-963` `.calc-v2-progress-labels{display:flex;justify-content:space-between;...}`. The middle label is centered by space-between distribution, not anchored to 56%.
...
- **Spec says:** §3 eyebrow line: `02 · CALCULATOR  |  JOB J-2419` with the `|` separator at same color and "8px gap on each side".
- **Code has:** `index.html:201` puts label, separator, and job code as three siblings inside a flex container with `gap:var(--v2-s2)` (8px). Renders 8px before and after. Matches spec.
...
- **Code has:** `app.js:678-680` replaces the middle text with `getSelectedPhases().slice(0,3).join(' / ')`. Uses `/` instead of `·` as the inner-text separator. `Stucco re-coat` is shown only when 0 phases are selected.
...
- **Code has:** `styles.css:27` sets `body{font-family:'DM Sans',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;font-size:15px;line-height:1.6;...}`.
...
- **Code has:** `styles.css:985` sets `.v2-topnav-indicator{display:none}` inside `@media(max-width:900px)`.
...
- **Code has:** `index.html:103-120` adds a `license-badge` ("TRIAL") and a `theme-toggle` button between the indicator and avatar.
...
- **Code:** `styles.css:547-549` renders three bars at 14px/11px/8px (a stepping pattern). Spec doesn't dictate, but does suggest "≡-like horizontal-bars icon" with equal-width bars.
...
- **Code:** `styles.css:550` sets `gap:4px`. Spec doesn't pin this exactly. Acceptable.

### [m3] PRO pill `padding:2px 6px` matches spec; OK.

### [m4] Topnav links gap is `--v2-s5` (20px), spec says "20-24px gap"

- **Code:** `styles.css:560` sets `gap:var(--v2-s5)` to 20px. At the low end of spec range. OK.
...
- **Code:** `styles.css:651` `.calc-v2-status-dot{...background:var(--v2-accent);...}`. Acceptable for Draft state. The badge text is hardcoded to "Draft" in HTML (`index.html:206`), matching the single-state implementation. Noted for future status variants.
...
- **Code:** `styles.css:708` removes focus ring with `.calc-v2-supplier-select:focus{outline:none}`. This is an accessibility regression. Add a focus-visible style like `outline 2px solid --v2-accent`.
...
The build is **approximately 85% complete**. The visual chrome (chip palette, type tokens, surfaces, hairlines) is solid and the right structure exists for every spec section. However, **four critical gaps** (C1-C4) need to be closed in the next pass:

...
.pac-item:hover,.pac-item-selected{
    background:var(--v2-bg-surface-hover,rgba(255,255,255,.04));
}
.pac-item-query{
...
.pac-matched{color:var(--v2-accent,#4a9eff)}
...
[data-theme="light"] .pac-item-selected{background:#f5f5f3}
[data-theme="light"] .pac-item-query{color:#111}
[data-theme="light"] .pac-matched{color:#0066cc}
```
...
```html
<div class="calc-v2-field"><label>Project Address</label><input type="text" class="calc-v2-input" id="calcProjectAddress" placeholder="e.g. 123 Main St, City, ST" data-on-input="updateCalcHeader"></div>
```
...
```html
                    <div class="calc-v2-form-row">
                        <div class="calc-v2-field calc-v2-field-full"><label>Delivery Notes</label><textarea class="calc-v2-input calc-v2-textarea" id="calcDeliveryNotes" rows="2" placeholder="Lot #, gate code, delivery instructions..."></textarea></div>
                    </div>
```
...
.calc-v2-field-full{grid-column:1/-1}
```
...
```js
document.getElementById('calcProjectAddress').value=job.isTemplate?'':job.projectAddress||'';
```
Add immediately after it:
```js
const dnEl=document.getElementById('calcDeliveryNotes');if(dnEl)dnEl.value=job.isTemplate?'':job.deliveryNotes||'';
```
...
```css
    .order-v2-notes-print{border-color:#bbb !important}
    .order-v2-notes-eyebrow{color:#555 !important}
    .order-v2-notes-body{color:#333 !important}
```

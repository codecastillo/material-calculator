...
// IMPORTANT: this route uses the raw body for signature verification, so it
// must be mounted with express.raw() (set in server.js). The route does NOT
// fall back to express.json().
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    try {
      ...
      const planLabel = plan === 'lifetime' ? 'Lifetime' : plan === 'yearly' ? 'Yearly' : 'Monthly';
      try {
    ...
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
        ...
        <tr><td style="padding:36px 32px 20px">
          ...
          <p style="margin:0 0 24px;color:#59636e;font-size:15px;line-height:1.5">Thanks for purchasing the ${planLabel} plan. Click below to finish setting up your account &mdash; takes about a minute.</p>

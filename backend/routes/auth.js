  ...
  try {
    ...
    if (req.body.referral_code) {
      const { data: referrer } = await supabase
        .from('users')
        .select('id')
        .eq('referral_code', req.body.referral_code)
        .single();
    ...
    const { data: user, error } = await supabase
      ...
      .insert(insertData)
      .select(
        'id, email, name, role, license_type, license_key, license_expires, is_active, email_verified'
      )
      .single();
  ...
  try {
    ...
    res.json({
      ...
      user: {
        ...
        license_expires: user.license_expires,
...
router.put('/profile', authenticate, async (req, res, next) => {
  try {
    ...
    const { data: user, error } = await supabase
      ...
      .update(updates)
      .eq('id', req.user.id)
      .select('id, email, name, role, license_type, license_expires, is_active')
      .single();
    if (error) throw error;
...
router.post('/activate', authenticate, async (req, res, next) => {
  try {
    ...
    if (licenseKey.times_used >= licenseKey.max_uses)
      return res.status(400).json({ error: 'Key has been used the maximum number of times' });
    ...
    await supabase
      .from('license_keys')
      .update({ times_used: licenseKey.times_used + 1 })
      .eq('id', licenseKey.id);

...
router.get('/me', authenticate, async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from('users')
      .select(
        'company_name, company_address, company_phone, company_email, contractor_license, onboarding_completed'
      )
      .eq('id', req.user.id)
      .single();
    if (error) {
  ...
  try {
    ...
    if (existing)
      return res.status(409).json({
        error:
          'An account already exists for this email. Sign in and activate your key on the account page.',
      });
    ...
    const { data: user, error: userErr } = await supabase
      ...
      .insert(insertData)
      .select(
        'id, email, name, role, license_type, license_key, license_expires, is_active, email_verified, onboarding_completed'
      )
      .single();
    ...
    await supabase
      .from('license_keys')
      .update({ times_used: (keyRow.times_used || 0) + 1 })
      .eq('id', keyRow.id);

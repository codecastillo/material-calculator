  ...
  try {
    ...
    const { data: material, error } = await supabase
      ...
      .insert({
        ...
        category_id,
        coverage_per_unit: coverage_per_unit || 100,
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (
      ...
      oldPrice !== null &&
      oldPrice !== updates.price_per_unit
    ) {
...
router.post('/bulk-price-update', async (req, res, next) => {
  try {
    ...
    if (!supplier_id || percentage === undefined)
      return res.status(400).json({ error: 'supplier_id and percentage required' });
    if (!(await verifySupplierOwnership(supplier_id, req.user.id)))
      return res.status(403).json({ error: 'Access denied' });
    ...
    for (const mat of materials || []) {
      ...
      const { error } = await supabase
        .from('materials')
        .update({ price_per_unit: newPrice, updated_at: new Date().toISOString() })
        .eq('id', mat.id);
      if (!error) updated++;
    }
    ...
    res.json({
      updated,
      message: `${updated} materials updated by ${percentage > 0 ? '+' : ''}${percentage}%`,
    });
  } catch (err) {
    next(err);
  }

...
async function seed() {
  ...
  if (!seedPassword) {
    console.error('Set SEED_ADMIN_PASSWORD env var before seeding');
    return;
  }
  ...
  if (existingUser) {
    ...
    const { data: newUser, error } = await supabase
      .from('users')
      .insert({ email: seedEmail, password_hash: hash, name: 'Admin' })
      .select('id')
      .single();
    if (error) {
  ...
  for (let i = 0; i < categoryNames.length; i++) {
    ...
    if (existing) {
      ...
      const { data: cat, error } = await supabase
        .from('categories')
        .insert({ name, user_id: userId, sort_order: i + 1 })
        .select('id')
        .single();
      if (error) {
  ...
  for (const sc of supplierConfigs) {
    ...
    if (existing) {
      ...
      const { data: sup, error } = await supabase
        .from('suppliers')
        .insert({ name: sc.name, user_id: userId })
        .select('id')
        .single();
      if (error) {
        console.error(`Supplier "${sc.name}" error:`, error.message);
        continue;
      }
      supId = sup.id;
  ...
  const allMaterials = [
    ...
    ...[
      ...
      {
        name: 'A-100 Exterior Latex (1 gal)',
        sku: 'SW-A100-1G',
        unit: 'gal',
        price: 42,
        category: 'Painting',
      ...
      {
        name: 'A-100 Exterior Latex (5 gal)',
        sku: 'SW-A100-5G',
        unit: 'bucket',
        price: 185,
        category: 'Painting',
  ...
  for (const mat of allMaterials) {
    ...
    if (error) {
      console.error(`Material "${mat.name}" error:`, error.message);
    } else created++;
  }
